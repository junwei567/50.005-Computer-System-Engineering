...
// encrypt each column or row bytes 
            byte[] encryptedWidthPixel = desCipher.doFinal(each_width_pixel);
            // System.out.println(each_width_pixel.length);
            // System.out.println(encryptedWidthPixel.length);

// convert the encrypted byte[] back into int[] and write to outImage (use setRGB)
            IntBuffer buff_int = ByteBuffer.wrap(encryptedWidthPixel).order(ByteOrder.BIG_ENDIAN).asIntBuffer();
            int[] arr = new int[buff_int.remaining()];
            buff_int.get(arr);
